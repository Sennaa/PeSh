...
#    #print index
#    shops_tags_shop      = [2] * len(tags_name)
...
#shops_similarities = [[] for x in range(len(shops_name))]
...
#        shop1tagslist = list(shop1tags)
#        shop2tags = shops_tags[shop2]
#        shop2tagslist = list(shop2tags)
#        # TODO: is this good?
#        # Amount of equal tags (1-1 or 0-0) / Total amount of tags
#        equals = [i for i in range(0,len(shop1tagslist)) if shop1tagslist[i] == shop2tagslist[i]]
#        equals_amount = len(equals)
#        total = len(tags_name)
#        shops_similarities_shop[shop2] = float(float(equals_amount) / float(total))
#    shops_similarities[shop1] = shops_similarities_shop
...
#    tags_similarities[tag1] = tags_similarities_tag

#tags_cols = ["name"] + ["tagsimilarities" + str(i) for i in range(len(tags_name))]
...
def convert_personas(id_persona, persona, name, description):
    ...
    for index, row in persona.iterrows():
        persona_postags = [0] * len(tags_name)
        persona_negtags = [0] * len(tags_name)
        persona_vislist = [0] * (len(row)-1)
        persona_ratlist = [0] * (len(row)-1)
        persona_shoplist = [0] * len(shops_name)
        ...
        if (index == 3): # vislist
            ...
            temp_list = ['%.0f' % float(vis) for vis in persona_vislist]
            print len(temp_list)
            temp_list = np.delete(temp_list, [43,92,93,94,295])
            temp_list = ['%.0f' % float(a) for a in temp_list]
            print len(temp_list)
            persona_vislists[persona_id] = ';'.join(temp_list)
        ...
        if (index == 4): # ratlist
            ...
            temp_list = ['%.3f' % float(rat) for rat in persona_ratlist]
            print len(temp_list)
            temp_list = np.delete(temp_list, [43,92,93,94,295])
            temp_list = ['%.3f' % float(a) for a in temp_list]
            print len(temp_list)
            persona_ratlists[persona_id] = ';'.join(temp_list)
...
## Shops
#df_shops = pd.DataFrame(np.column_stack((shops_name, shops_address, shops_time, shops_tags)))
#df_shops.to_csv('DATA_shops_new.csv', sep='^', index=False)

## ShopSimilarities
#df_shopsimilarities = pd.DataFrame(np.column_stack((shops_name,shops_similarities)))
#df_shopsimilarities.to_csv('DATA_shopsimilarities_new.csv', sep='^', index=False)
